...
func NewTelemetryBuilder(settings component.TelemetrySettings, options ...TelemetryBuilderOption) (*TelemetryBuilder, error) {
	...
	var err, errs error
	builder.ConnectorGroupbytraceConfNumTraces, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Gauge(
	...
	builder.ConnectorGroupbytraceEventLatency, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Histogram(
	...
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceIncompleteReleases, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Counter(
	...
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceNumEventsInQueue, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Gauge(
		"otelcol_connector_groupbytrace_num_events_in_queue",
		metric.WithDescription("Number of events currently in the queue"),
		metric.WithUnit("1"),
	)
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceNumTracesInMemory, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Gauge(
	...
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceSpansReleased, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Counter(
		"otelcol_connector_groupbytrace_spans_released",
		metric.WithDescription("Spans released to the next consumer"),
		metric.WithUnit("1"),
	)
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceTracesEvicted, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Counter(
		"otelcol_connector_groupbytrace_traces_evicted",
		metric.WithDescription("Traces evicted from the internal buffer"),
		metric.WithUnit("1"),
	)
	errs = errors.Join(errs, err)
	builder.ConnectorGroupbytraceTracesReleased, err = getLeveledMeter(builder.meter, configtelemetry.LevelBasic, settings.MetricsLevel).Int64Counter(
