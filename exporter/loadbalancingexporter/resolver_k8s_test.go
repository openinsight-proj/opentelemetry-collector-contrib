...
func TestK8sResolve(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			simulateFn: func(ctx *suiteContext, args args) error {
				endpoint, exist := ctx.endpoint.DeepCopy(), ctx.endpoint.DeepCopy()
				...
				_, err = ctx.clientset.CoreV1().Endpoints(args.namespace).
			...
			verifyFn: func(ctx *suiteContext, args args) error {
				...
				if !slices.Equal([]string{
					...
					"192.168.10.100:8080",
					"192.168.10.100:9090",
				}, ctx.resolver.endpoints) {
					return errors.New("resolver failed, endpoints not equal")
				}
				return nil
			},
		},
		{
			...
			verifyFn: func(suiteCtx *suiteContext, args args) error {
				...
				if len(suiteCtx.resolver.endpoints) != 0 {
					return errors.New("resolver failed, endpoints should empty")
				}
				return nil
			},
		},
